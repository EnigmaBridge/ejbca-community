 ...
 *
 * @version $Id: OcspMonitoringTool.java 19902 2014-09-30 14:32:24Z anatom $
 */
public class OcspMonitoringTool extends DatabaseCliCommand {
	...
	final String usage = "\n"
		+ getSubCommand() + " [-ns] <inclusion-mode> <batchSize> <timeToConfirmError> <certificateProfileId1> [<certificateProfileId2>] ... - <master persistence unit> <slave1 persistence unit> [<slave2 persistence unit>] ...\n"
		+ " Compares different VA/OCSP databases to the CA's database and reports discrepancies.\n"
		+ " Persistence units are configured in META-INF/persistence.xml.\n"
		+ " Used JDBC drivers JARs must be present in endorsed/.\n\n"
		...
		+ " -                         required separator\n"
		+ " master persistence unit:  persistence-unit name of CA database in the persistence.xml file\n"
		+ " slave1 persistence unit:  persistence-unit name of the first OCSP database in the persistence.xml file\n"
	    + "\nCheck ctb.log for messages.";

    @Override
    public String getDescription() {
        return "Compares different VA/OCSP databases to the CA's database and reports certificate status discrepancies.";
    }
    ...
    public int executeInternal(String[] args) throws Exception {
    	log.info("Monitoring tool started with " + Arrays.toString(args));
    	...
    	final EntityManager caEntityManager = getEntityManager(args[currentArg]);
    	final String caEntityManagerName = args[currentArg];
    	log.info("Using \"" + caEntityManagerName + "\" as persistence unit reference to the CA database.");
    	currentArg++;
    	...
    	for (int i=currentArg; i<args.length; i++) {
    		...
    		ocspEntityManagers.add(getEntityManager(args[i]));
    	}
    	log.info("Added " + ocspEntityManagers.size() + " OCSP persistence unit(s)");
